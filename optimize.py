import numpy, urllib.request, re, math, operator, functools
from scipy import optimize
r = float(re.findall('<td class=\"text_view_data\">(.+?)</td>', ''.join([x.decode('utf-8').strip() for x in urllib.request.urlopen('https://www.treasury.gov/resource-center/data-chart-center/interest-rates/Pages/TextView.aspx?data=yield')]))[-10])/100
...
def evaluate(mu, sigma, R):
	...
	f, ef = min(1, kelly(portfolio_mu, mu, sigma, R, ef)), eff_front(portfolio_mu, mu, sigma, R)
	return {'weight': [f*x for x in ef[0]], 'portfolio expected return': f*portfolio_mu[0], 'portfolio volatility': f*math.sqrt(ef[1])}